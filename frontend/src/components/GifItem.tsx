...
const GifItem = ({ id, url, title, tags, likes, likedBy, uploadedBy }: GifItemProps) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="absolute bottom-0 left-0 right-0 bg-black/25 text-white p-2 rounded-b-2xl
      ...
      <div
        className="absolute top-2 right-2 bg-black/15 flex justify-center items-center
      ...
      >
        ...
        <button onClick={() => navigator.clipboard.writeText(url)} className="p-2 cursor-pointer transition-transform duration-200 hover:scale-110">
